...
def lift_gaussian(d, t_mean, t_var, r_var, diag):
    ...
    d_mag_sq = torch.clamp(torch.sum(d**2, dim=-1, keepdim=True), min=1e-10)
...
def conical_frustum_to_gaussian(d, t0, t1, base_radius, diag, stable=True):
    ...
    if stable:
        ...
        t_mean = mu + (2 * mu * hw**2) / torch.clamp(
            3 * mu**2 + hw**2, min=eps
        )
...
def volumetric_rendering(
    ...
    if compute_extras:
        ...
        expectation = lambda x: (weights * x).sum(dim=-1) / torch.clamp(
            acc, min=eps
        )
        ...
        distance_percentiles = stepfun.weighted_percentile(
            t_aug,
            weights_aug,
            torch.tensor(ps, dtype=torch.float32, device=tdist.device),
        )

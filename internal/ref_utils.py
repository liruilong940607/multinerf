...
def reflect(viewdirs, normals):
    ...
    """
    return (
        2.0 * torch.sum(normals * viewdirs, dim=-1, keepdim=True) * normals
        - viewdirs
    )
...
def l2_normalize(x, eps=torch.finfo(torch.float32).eps):
    ...
    return x / torch.sqrt(
        torch.clamp(torch.sum(x**2, dim=-1, keepdim=True), min=eps)
    )
...
def compute_weighted_mae(weights, normals, normals_gt):
    ...
    return (
        (
            weights
            * torch.arccos(
                torch.clamp((normals * normals_gt).sum(-1), -one_eps, one_eps)
            )
        ).sum()
        / weights.sum()
        * 180.0
...
def generalized_binomial_coeff(a, k):
    ...
    return torch.prod(a - torch.arange(k)) / np.math.factorial(k)
...
def generate_ide_fn(deg_view):
    ...
    def integrated_dir_enc_fn(xyz, kappa_inv):
        ...
        """
        _mat = mat.to(xyz.device)
        x = xyz[..., 0:1]
        y = xyz[..., 1:2]
        z = xyz[..., 2:3]

        # Compute z Vandermonde matrix.
        vmz = torch.cat([z**i for i in range(_mat.shape[0])], dim=-1)

        # Compute x+iy Vandermonde matrix.
        vmxy = torch.cat([(x + 1j * y) ** m for m in ml_array[0, :]], dim=-1)

        # Get spherical harmonics.
        sph_harms = vmxy * torch.matmul(vmz, _mat)

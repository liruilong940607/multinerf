...
from . import camera_utils, configs
...
class NeRFSceneManager(pycolmap.SceneManager):
    ...
    def process(
        ...
        pixtocam = torch.linalg.inv(
            camera_utils.intrinsic_matrix(fx, fy, cx, cy)
        )
        ...
        c2w_mats = torch.linalg.inv(w2c_mats)
        poses = c2w_mats[:, :3, :4]
...
def load_blender_posedata(data_dir, split=None):
    ...
    w = meta["w"]
    h = meta["h"]
    cx = meta["cx"] if "cx" in meta else w / 2.0
    cy = meta["cy"] if "cy" in meta else h / 2.0
    if "fl_x" in meta:
        fx = meta["fl_x"]
    else:
    ...
    pixtocam = torch.linalg.inv(camera_utils.intrinsic_matrix(fx, fy, cx, cy))
    coeffs = ["k1", "k2", "p1", "p2"]
    if not any([c in meta for c in coeffs]):
        params = None
    else:
        params = {c: (meta[c] if c in meta else 0.0) for c in coeffs}
    camtype = camera_utils.ProjectionType.PERSPECTIVE
    return names, poses, pixtocam, params, camtype
...
class Dataset(threading.Thread, metaclass=abc.ABCMeta):
    ...
    Attributes:
      ...
      images: Tensor, array of RGB image data.
      metadata: dict, optional metadata for raw datasets.
      near: float, near plane value for rays.
      normal_images: Tensor, optional array of surface normal vector data.
      pixtocams: Tensor, one or a list of inverse intrinsic camera matrices.
      pixtocam_ndc: Tensor, the inverse intrinsic matrix used for NDC space.
      poses: Tensor, optional array of auxiliary camera pose data.
    ...
    def __init__(self, split: str, data_dir: str, config: configs.Config):
        ...
        self.images: Tensor = None
        self.camtoworlds: Tensor = None
        self.pixtocams: Tensor = None
        ...
        if self.render_path:
            ...
            self.distortion_params = None
            self.pixtocams = camera_utils.get_pixtocam(
    ...
    def _make_ray_batch(
        ...
        if (
            ...
            rays = camera_utils.cast_ray_batch(
                self.cameras, pixels, self.camtype
            )
    ...
    def _next_train(self) -> utils.Batch:
        ...
        if self._batching == utils.BatchingMethod.ALL_IMAGES:
            cam_idx = torch.randint(0, self._n_examples, (num_patches, 1, 1))
        else:
            cam_idx = torch.randint(0, self._n_examples, (1,))
    ...
    def _load_renderings(self, config):
        ...
        for _, frame in enumerate(meta["frames"]):
            ...
            if self._use_tiffs:
                ...
                image = lib_image.linear_to_srgb(torch.stack(channels, dim=-1))
            else:
            ...
            if self._load_normals:
                normal_image = (
                    get_img("_normal.png")[..., :3] * 2.0 / 255.0 - 1.0
                )
        ...
        if self._load_disps:
            self.disp_images = torch.stack(disp_images, dim=0)
        if self._load_normals:
            self.normal_images = torch.stack(normal_images, dim=0)
        ...
        self.camtoworlds = torch.stack(cams, dim=0)
        self.focal = (
            0.5 * self.width / np.tan(0.5 * float(meta["camera_angle_x"]))
        )
        self.pixtocams = camera_utils.get_pixtocam(
            self.focal, self.width, self.height
        )
    ...
    def _load_renderings(self, config):
        ...
        if config.forward_facing:
            ...
            if config.render_spline_keyframes is not None:
                rets = camera_utils.create_render_spline_path(
                    config, image_names, poses, self.exposures
                )
                (
                    self.spline_indices,
                    self.render_poses,
                    self.render_exposures,
                ) = rets
            else:
        ...
        # Select the split.
        all_indices = torch.arange(images.shape[0])
...
class TanksAndTemplesNerfPP(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        poses = torch.matmul(
            poses, torch.diag(torch.tensor([1.0, -1.0, -1.0, 1.0]))
        )
        ...
        if not config.render_path:
            images = (
                load_files("rgb", lambda f: np.array(Image.open(f))) / 255.0
            )
            self.images = images
            self.height, self.width = self.images.shape[1:3]
...
class TanksAndTemplesFVS(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        sizes = [
            f for f in sorted(utils.listdir(basedir)) if f.startswith("ibr3d")
        ]
        sizes = sizes[::-1]
        ...
        files = [
            f for f in sorted(utils.listdir(basedir)) if f.startswith("im_")
        ]
        if render_only:
            files = files[:1]
        images = (
        ...
        c2w_colmap = torch.linalg.inv(camera_utils.pad_poses(w2c))[:, :3, :4]
        c2w = c2w_colmap @ torch.diag(torch.tensor([1.0, -1.0, -1.0, 1.0]))
...
class DTU(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        for i in range(1, n_images + 1):
            ...
            if config.factor > 0:
                ...
                camera_mat = (
                    torch.diag(
                        torch.tensor(
                            [1.0 / config.factor, 1.0 / config.factor, 1.0]
                        )
                    )
                    @ camera_mat
                )
            pixtocams.append(torch.linalg.inv(camera_mat))

        pixtocams = torch.stack(pixtocams)
        camtoworlds = torch.stack(camtoworlds)
        images = torch.stack(images)

        def rescale_poses(poses):
            """Rescales camera poses according to maximum x/y/z value."""
            s = torch.max(torch.abs(poses[:, :3, -1]))
            out = poses.clone()
            out[:, :3, -1] /= s
            return out
        ...
        camtoworlds = camtoworlds @ torch.diag(

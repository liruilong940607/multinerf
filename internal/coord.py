...
def contract(x):
    ...
    x_mag_sq = torch.clamp(torch.sum(x**2, dim=-1, keepdim=True), min=eps)
    # This is a hack to make torch.func.linearize() happy.
    two = torch.tensor(2, dtype=torch.float32, device=x.device)
    z = torch.where(
        x_mag_sq <= 1, x, ((two * torch.sqrt(x_mag_sq) - 1) / x_mag_sq) * x
    )
    return z
...
def inv_contract(z):
    ...
    z_mag_sq = torch.clamp(torch.sum(z**2, dim=-1, keepdim=True), min=eps)
...
def track_linearize(fn, mean, cov):
    ...
    # Note torch.func.linearize() currently triggers a UserWarning from `const_fold()`.
    # https://github.com/pytorch/pytorch/issues/94374
    fn_mean, lin_fn = torch.func.linearize(fn, mean)
...
def integrated_pos_enc(mean, var, min_deg, max_deg):
    ...
    return expected_sin(
        torch.cat([scaled_mean, scaled_mean + 0.5 * math.pi], dim=-1),
...
def pos_enc(x, min_deg, max_deg, append_identity=True):
    ...
    four_feat = torch.sin(
        torch.cat([scaled_x, scaled_x + 0.5 * math.pi], dim=-1)
    )
    if append_identity:

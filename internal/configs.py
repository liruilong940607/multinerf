...
configurables = {
    ...
    "torch.nn": [F.relu, F.softplus, F.silu],
}
...
class Config:
    ...
    batching: str = (
        "all_images"  # Batch composition, [single_image, all_images].
    )
    ...
    forward_facing: bool = (
        False  # Set to True for forward-facing LLFF captures.
    )
    render_path: bool = False  # If True, render a path. Used only by LLFF.
    llffhold: int = (
        8  # Use every Nth image for the test set. Used only by LLFF.
    )
    ...
    compute_disp_metrics: bool = (
        False  # If True, load and compute disparity MSE.
    )
    compute_normal_metrics: bool = (
        False  # If True, load and compute normal MAE.
    )
    # In PyTorch we need the garbage collector to run to avoid OOMs.
    ...
    max_steps: int = 2000000  # The number of optimization steps.
    ...
    print_every: int = (
        100  # The number of steps between reports to tensorboard.
    )
    train_render_every: int = (
        5000  # Steps between test set renders when training
    )
    cast_rays_in_train_step: bool = (
        False  # If True, compute rays in train step.
    )
    data_loss_type: str = (
        "charb"  # What kind of loss to use ('mse' or 'charb').
    )
    ...
    weight_decay_mults: Dict[str, Any] = field(
    ...
    lr_init: float = 0.00025  # The initial learning rate.
    lr_final: float = 0.0000025  # The final learning rate.
    ...
    grad_max_norm: float = (
        0.001  # Gradient clipping magnitude, disabled if == 0.
    )
    ...
    eval_only_once: bool = (
        True  # If True evaluate the model only once, ow loop.
    )
    ...
    eval_dataset_limit: int = torch.iinfo(
        torch.int32
    ).max  # Num test images to eval.
    eval_quantize_metrics: bool = True  # If True, run metrics on 8-bit images.
    eval_crop_borders: int = (
        0  # Ignore c border pixels in eval (x[c:-c, c:-c]).
    )
    ...
    render_dist_percentile: float = (
        0.5  # How much to trim from near/far planes.
    )
    ...
    render_spline_keyframes: Optional[
        str
    ] = None  # Text file containing names of
    ...
    render_spline_degree: int = (
        5  # Polynomial degree of B-spline interpolation.
    )
    render_spline_smoothness: float = 0.03  # B-spline smoothing factor, 0 for
    ...
    num_border_pixels_to_mask: int = (
        0  # During training, discard N-pixel border
    )
...
def load_config(save_config=True):
    ...
    config = Config()
    if save_config and torch.cuda.current_device() == 0:

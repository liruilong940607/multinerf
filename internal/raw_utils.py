...
def pixels_to_bayer_mask(pix_x: Tensor, pix_y: Tensor) -> Tensor:
    ...
    g = (pix_x % 2 == 1) * (pix_y % 2 == 0) + (pix_x % 2 == 0) * (
        pix_y % 2 == 1
    )
    # Blue is bottom right (1, 1).
    b = (pix_x % 2 == 1) * (pix_y % 2 == 1)
    return torch.stack([r, g, b], -1).float()


def bilinear_demosaic(bayer: Tensor) -> Tensor:
    ...
    def upsample_green(g1, g2):
        ...
        for i in range(4):
            axis = -1 - (i // 2)
            roll = -1 + 2 * (i % 2)
            alt = alt + 0.25 * torch.roll(z, roll, dim=axis)
...
def load_raw_images(
    image_dir: str, image_names: Optional[Sequence[str]] = None
    ...
    raws, exifs = zip(*data)
    raws = torch.stack(raws, dim=0).float()
...
def process_exif(
    exifs: Sequence[Mapping[str, Any]]
    ...
    for key in _EXIF_KEYS:
        ...
        meta[key] = torch.squeeze(torch.tensor(vals))
    # Shutter speed is a special case, a string written like 1/N.
    meta["ShutterSpeed"] = torch.tensor(
        (1.0 / float(exif["ShutterSpeed"].split("/")[1]) for exif in exifs),
        dtype=torch.float32,
    )
    ...
    cam2camwb = torch.tensor([np.diag(1.0 / x) for x in whitebalance])
    ...
    rgb2camwb /= rgb2camwb.sum(dim=-1, keepdim=True)
    # Combining color correction with white balance gives the entire transform.
    cam2rgb = torch.linalg.inv(rgb2camwb) @ cam2camwb
...
def load_raw_dataset(
    ...
    n_downsample: int,
) -> Tuple[Tensor, MutableMapping[str, Any], bool]:
    ...
    if testscene and split == utils.DataSplit.TEST:
        ...
        testraw = torch.from_numpy(testraw.astype(np.float32) / 4.0)
    ...
    unique_shutters = torch.sort(torch.unique(shutter_speeds))[0][::-1]
    exposure_idx = torch.zeros_like(shutter_speeds, dtype=torch.int)
    ...
    image0_raw_demosaic = bilinear_demosaic(images[0])
    image0_rgb = image0_raw_demosaic @ meta["cam2rgb"][0].T
    exposure = torch.quantile(image0_rgb, exposure_percentile / 100.0)
    meta["exposure"] = exposure
    # Sweep over various exposure percentiles to visualize in training logs.
    exposure_levels = {
        p: torch.quantile(image0_rgb, p / 100.0) for p in _PERCENTILE_LIST
    }
    ...
    meta["postprocess_fn"] = lambda z, x=exposure: postprocess_raw(
        z, cam2rgb0, x
    )
    ...
    def processing_fn(x):
        x_demosaic = bilinear_demosaic(x)
        if n_downsample > 1:
            x_demosaic = lib_image.downsample(x_demosaic, n_downsample)
        return x_demosaic

    images = torch.stack([processing_fn(im) for im in images], dim=0)

    return images, meta, testscene


def best_fit_affine(x: Tensor, y: Tensor, axis: _Axis) -> Tensor:
...
def match_images_affine(
    est: Tensor, gt: Tensor, axis: _Axis = (0, 1)
) -> Tensor:
    ...
    a, b = best_fit_affine(gt, est, dim=axis)

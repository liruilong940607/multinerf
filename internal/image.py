...
def mse_to_psnr(mse: Union[Tensor, float, np.ndarray]) -> Any:
    """Compute PSNR given an MSE (we assume the maximum pixel value is 1)."""
    if isinstance(mse, Tensor):
        return -10.0 / np.log(10.0) * torch.log(mse)
    else:
        return -10.0 / np.log(10.0) * np.log(mse)


def psnr_to_mse(psnr: Union[Tensor, float, np.ndarray]) -> Any:
    """Compute MSE given a PSNR (we assume the maximum pixel value is 1)."""
    if isinstance(psnr, Tensor):
        return torch.exp(-0.1 * np.log(10.0) * psnr)
    else:
...
def linear_to_srgb(
    linear: Tensor, eps: Optional[float] = torch.finfo(torch.float32).eps
) -> Tensor:
...
def srgb_to_linear(
    srgb: Tensor, eps: Optional[float] = torch.finfo(torch.float32).eps
) -> Tensor:
...
def downsample(img, factor):
    ...
    img = img.reshape(
        (sh[0] // factor, factor, sh[1] // factor, factor) + sh[2:]
    )
    img = img.mean((1, 3))
    return img

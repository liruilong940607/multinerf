...
def query(tq, t, y, outside_value=0):
    ...
    tq = torch.broadcast_to(tq, t.shape[:-1] + tq.shape[-1:])
    idx_hi = torch.searchsorted(t.contiguous(), tq.contiguous(), right=True)
    idx_lo = torch.clamp(idx_hi - 1, min=0, max=y.shape[-1] - 1)
    selector = (idx_hi < t.shape[-1]) & (idx_hi > 0)
    yq = torch.where(selector, y.gather(-1, idx_lo), outside_value)
    return yq
...
def weighted_percentile(t, w, ps):
    ...
    fn = lambda cw_i, t_i: math.interp(ps / 100, cw_i, t_i)
